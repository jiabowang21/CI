...
// Dibuixa la portada del joc
void dibuixar_portada () {
   int numberPosi = 0;
   int numberPosy = 0;
   int aux;
   int contador = 0;
   for (numberPosi = 0; numberPosi < 8; ++numberPosi) { 
      for (numberPosy = 0; numberPosy < 128; ++numberPosy) { 
	aux = bitmap[contador];
	++contador;
	writeByte(numberPosi, numberPosy, aux);
      }
   }
}

/* Escriu el número k a la pàgina (de 0 a 7), character column y (de 0 a 20)
L'enter k, només pot ser del 0 al 9 */
void writenumbers(byte p, byte y, int k)
{
   int numberPosi = k*44; //cada número ocupa 44 words, per tant, la seva posició es el número*44
   int aux;
   byte aoy = y;
   for (numberPosi = k*44; numberPosi < ((k*44)+44); numberPosi += 11) { //el bucle comença a número*44 i acaba a (número*44)+44, es a dir, un bucle de 44 iteracions
      byte aoy = y;
      for (int numberPosy = 0; numberPosy < 11; ++numberPosy) { //cada fila té 11 columnes
	aux = numero[numberPosi + numberPosy];
	writeByte (p, aoy, aux);
	++aoy;
      }
      ++p;
